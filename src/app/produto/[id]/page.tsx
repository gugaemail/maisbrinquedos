    ...
    <>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <nav className="flex items-center gap-2 text-sm text-[#6B7080] dark:text-white/60 mb-8 font-body">
          ...
          <span>/</span>
          <span className="text-[#0F0F0F] dark:text-white">{product.name}</span>
        </nav>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          <div className="aspect-square rounded-3xl flex items-center justify-center text-9xl relative overflow-hidden" style={{ backgroundColor: CATEGORY_BG[product.category.name] ?? "#FFE14D" }}>
            {imageUrl ? (
            ...
            {product.tag && (
              <span className="absolute top-4 left-4 px-3 py-1 rounded-full bg-[#3B8BFF] text-white text-sm font-semibold">
                {product.tag}
              </span>
            )}
          </div>
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-2">
              <BuyNowButton product={{ id: product.id, name: product.name, price, emoji, imageUrl: imageUrl ?? undefined }} />
