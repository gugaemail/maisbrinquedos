  ...
  return (
    <div className="p-8 max-w-3xl">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-display font-extrabold text-[#0F0F0F]">Pedido</h1>
          <p className="text-xs text-[#6B7080] font-mono mt-0.5">{order.id}</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="bg-white rounded-2xl border border-[#E2E6F0] p-6 space-y-1">
          <p className="text-xs text-[#6B7080] font-semibold uppercase tracking-wide mb-2">Cliente</p>
          <p className="font-semibold text-[#0F0F0F]">{order.payerName}</p>
          <p className="text-sm text-[#6B7080]">{order.payerEmail}</p>
          {formatCPF(order.payerCpf) && (
            <p className="text-sm text-[#6B7080]">CPF: {formatCPF(order.payerCpf)}</p>
          )}
          {formatPhone(order.payerPhone) && (
            <p className="text-sm text-[#6B7080]">Tel: {formatPhone(order.payerPhone)}</p>
          )}
        </div>

        <div className="bg-white rounded-2xl border border-[#E2E6F0] p-6 space-y-1">
          <p className="text-xs text-[#6B7080] font-semibold uppercase tracking-wide mb-2">Pagamento</p>
          <p className="text-2xl font-display font-extrabold text-[#0F0F0F]">R$ {fmt(total)}</p>
          {order.mpPaymentId && (
            <p className="text-xs text-[#6B7080] font-mono">Pagamento MP: {order.mpPaymentId}</p>
          )}
          {order.mpPreferenceId && (
            <p className="text-xs text-[#6B7080] font-mono">Preferência MP: {order.mpPreferenceId}</p>
          )}
        </div>
      </div>
      ...
      {/* Endereço de entrega */}
      {hasShipping && (
        <div className="bg-white rounded-2xl border border-[#E2E6F0] p-6 mb-4">
          <p className="text-xs text-[#6B7080] font-semibold uppercase tracking-wide mb-3">Endereço de entrega</p>
          <div className="space-y-0.5">
            {addressParts.map((line, i) => (
              <p key={i} className="text-sm text-[#0F0F0F]">{line}</p>
            ))}
          </div>
        </div>
      )}
        ...
        <table className="w-full text-sm font-body">
          ...
          <tbody className="divide-y divide-[#E2E6F0]">
            {order.items.map((item) => (
              <tr key={item.id}>
                <td className="px-6 py-4 font-semibold text-[#0F0F0F]">{item.productName}</td>
                <td className="px-4 py-4 text-center text-[#6B7080]">{item.quantity}</td>
                <td className="px-4 py-4 text-right text-[#6B7080]">R$ {fmt(Number(item.unitPrice))}</td>
                <td className="px-6 py-4 text-right font-semibold text-[#0F0F0F]">
                  R$ {fmt(Number(item.unitPrice) * item.quantity)}
                </td>
              </tr>
            ))}
          </tbody>
          <tfoot className="border-t-2 border-[#E2E6F0] bg-[#F8F9FC]">
            <tr>
              <td colSpan={3} className="px-6 py-4 text-right text-sm font-semibold text-[#6B7080]">Total</td>
              <td className="px-6 py-4 text-right text-base font-extrabold text-[#0F0F0F]">R$ {fmt(total)}</td>
            </tr>
          </tfoot>
        </table>
      </div>
